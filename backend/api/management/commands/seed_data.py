...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        p_garlic = Post.objects.create(
            farmer=f1, product_type=product_type_map.get("Garlic"),
            title="Deshi Organic Sun-dried Garlic",
            price_per_kg=120.00, total_weight_kg=500.00,
            description="Premium quality sun-dried local garlic.",
            location=union_obj(3282),
        ...
        p_banana = Post.objects.create(
            farmer=f1, product_type=product_type_map.get("Raw Banana"),
            title="Sagar Banana (Medium Size)",
            price_per_kg=40.00, total_weight_kg=500.00,
        ...
        p_carrot = Post.objects.create(
            farmer=f1, product_type=product_type_map.get("Carrot"),
            title="Fresh Spring Carrots (Grade A)",
            price_per_kg=60.00, total_weight_kg=400.00,
        ...
        p_cucumber = Post.objects.create(
            farmer=f2, product_type=product_type_map.get("Cucumber"),
            title="Green Salad Cucumber Bulk",
            price_per_kg=42.00, total_weight_kg=1000.00,
            description="Standard size greenhouse grown salad cucumbers.",
            location=union_obj(1194),
            collection_point_address=f2.address,
            image=get_image_file("cucumber.jpg"),
        )
        p_potato = Post.objects.create(
            farmer=f3, product_type=product_type_map.get("Potato"),
            title="Rajshahi Potato (Diamond)",
            price_per_kg=30.00, total_weight_kg=2000.00,
            description="High-yield diamond variety potatoes from Rajshahi.",
            location=union_obj(1216),
            collection_point_address=f3.address,
            image=get_image_file("potato.jpg"),
        )
        p_onion = Post.objects.create(
            farmer=f4, product_type=product_type_map.get("Onion"),
            title="Jashore Red Onion",
            price_per_kg=45.00, total_weight_kg=1500.00,
            description="Premium red onions from Jashore.",
            location=union_obj(1596),
            collection_point_address=f4.address,
            image=get_image_file("onion1.jpeg"),
        )
        p_melon = Post.objects.create(
            farmer=f5, product_type=product_type_map.get("Melon"),
            title="Sweet Red Watermelon",
            price_per_kg=35.00, total_weight_kg=1500.00,
        ...
        # 5) BELOW-THRESHOLD pool (Bogura Cucumber threshold 500kg -> pool only, no batch)
        make_order(c2, p_cucumber, 50)

        # 6) Additional completed orders with successful payments for settlement/BEFTN testing
